...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    # build-clean: deep clean the kernel build tree
    make ARCH=${ARCH} \
         CROSS_COMPILE=${CROSS_COMPILE} \
         mrproper
    # build-defconfig: configule for "virt" arm dev board
    make ARCH=${ARCH} \
         CROSS_COMPILE=${CROSS_COMPILE} \
         defconfig
    # build-vmlinux: build a kernel image for bootiong with QEMU
    make -j4 \
         ARCH=${ARCH} \
         CROSS_COMPILE=${CROSS_COMPILE} \
         all
...
#~ sudo cp -a ld-2.31.so        ld-linux-aarch64.so.1
#~ sudo cp -a libm-2.31.so      libm.so.6
#~ sudo cp -a libresolv-2.31.so libresolv.so.2
#~ sudo cp -a libc-2.31.so      libc.so.6
